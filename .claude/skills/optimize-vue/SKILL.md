...
| P1.6 | `<script setup>` component calls `emit(...)` but has no `defineEmits` declaration — emitted events become undocumented and break TypeScript inference | (1) `rg -l '<script setup' client/src` to find `<script setup>` files. (2) For each, check if both `\bemit\(` and `defineEmits\(` appear. Flag files where `emit(` is called but `defineEmits` is absent. |
| P1.7 | Emitted event name not in `defineEmits` declaration — Vue emits a runtime warning and downstream listeners may break | Per-file inspection of `<script setup>` files: extract event names from `defineEmits(['a', 'b', ...])` *or* `defineEmits<{ a: [...]; b: [...] }>()`, then diff against names in `emit('xxx', …)` calls. Flag any `emit('xxx')` whose `'xxx'` isn't in the declared set. (Account for kebab-case ↔ camelCase: `emit('my-event')` and `defineEmits(['myEvent'])` *may* match depending on the project's convention — flag the mismatch and let the reader decide.) |
| P1.8 | `<script setup>` references `props.X` but has no `defineProps` declaration — `props` is `undefined`, runtime error on access | (1) `rg -l '<script setup' client/src`. (2) For each, check if `\bprops\.\w+` appears in the `<script setup>` block but `defineProps\(` does not. Flag. |
